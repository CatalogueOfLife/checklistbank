...
import { ArrowRightOutlined, CodeOutlined, LoadingOutlined, DiffOutlined } from '@ant-design/icons';
...
const getPreviousFinishedImport = (importHistory, index) => {
  ...
  return _.get(prevHistory.filter(h => h.state === "finished"), '[0].attempt')
}
      ...
      <Timeline.Item key={h.attempt} color={tagColors[h.state]} dot={getDot(h, attempt)}>
        ...
        >
          ...
          {_.get(h, 'user.username') && <p>{_.get(h, 'upload') ? "Upload" : "Download"} by {h.user.username}</p>}
            <p><span style={{fontSize: "10px"}}>{`${moment(h.started).format("lll")}`} {" "}</span> <Tooltip title="Kibana logs" placement="right"><a href={kibanaQuery(h.datasetKey, h.attempt)} target="_blank" ><CodeOutlined /></a></Tooltip>
            {h.state === "finished" && getPreviousFinishedImport(importHistory, index) &&  <React.Fragment> {" "}
            <Tooltip title="Diff between this and previous attempt" placement="right">
            <NavLink
          to={{
            pathname: `/dataset/${h.datasetKey}/diff`,
            search: `?attempts=${getPreviousFinishedImport(importHistory, index)}..${h.attempt}`
          }}
          exact={true}
        >
          <DiffOutlined />
        ...
        {h.state === "failed" && (
          <React.Fragment>
            ...
            {_.get(h, 'user.username') && <p>Created by {h.user.username}</p>}
              <p style={{fontSize: "10px"}}>{`${moment(h.started).format("lll")}`}</p>
