...
class CslReferences extends React.Component {
  constructor(props) {
    super(props);
    const defaultStyleKey = localStorage.getItem("colplus_preferred_csl_style") || "council-of-science-editors";
    this.state = { csl: "", style: styles[defaultStyleKey], styleKey: defaultStyleKey };
  }
  ...
  componentDidMount = () => {
    ...
    this.processCsl(references, this.state.style).then(csl => this.setState({ csl }));
  };
  ...
  onStyleChange = (evt) => {
    ...
    const { references } = this.props;
    this.setState({style: styles[styleKey], styleKey: styleKey}, ()=> this.processCsl(references, this.state.style).then(csl => this.setState({ csl })))
  ...
  render = () => {
    ...
    return (
      <Row style={{marginLeft: '-60px', marginTop: '-8px'}}>
        <Col span={22}>
          <div dangerouslySetInnerHTML={{ __html: csl }} />
        </Col>
        <Col span={2} style={{textAlign: 'right'}}>
        <Popover placement="leftTop" title="Csl style" content={
          <RadioGroup onChange={this.onStyleChange} value={this.state.styleKey}>
          {Object.keys(styles).sort().map(key => <Radio style={radioStyle} value={key}>{key.replace(/[_-]/g, " ")}</Radio>)}
