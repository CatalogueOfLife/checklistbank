      ...
      try {
        ...
        if (!primaryKey) {
          ...
          const terms = foreignKeys.map(
            (p) =>
              `${p.split(".")[1]}=${encodeURIComponent(
                record?.terms?.[primaryKey]
              )}`
          );
          const linkedRecordsRes = await axios(
            `${config.dataApi}dataset/${
              datasetKey || sourceDatasetKey
            }/verbatim?${types.join("&")}&${terms.join(
              "&"
            )}&termOp=OR&limit=1000`
          );
          setVerbatimRecords(
            _.groupBy(
              [
                record,
                ...(linkedRecordsRes?.data?.result?.filter(
                  (r) => r.id !== record.id && r.file !== record.file
                ) || []),
              ],
              "type"
            )
