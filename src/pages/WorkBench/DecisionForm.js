...
const DecisionForm = (props) => {
   ...
   const handleSubmit = (next, cb) => {
    ...
    const getInitialValues = () => {
      const currentDecision = _.get(rowsForEdit[current], 'decisions[0]' );
      return {
        scientificName: (_.get(currentDecision, 'name.scientificName')) ? _.get(currentDecision, 'name.scientificName') : '',
        authorship: (_.get(currentDecision, 'name.authorship')) ? _.get(currentDecision, 'name.authorship') : '',
        rank: (_.get(currentDecision, 'name.rank')) ? _.get(currentDecision, 'name.rank') : "",
        status: (_.get(currentDecision, 'status')) ? _.get(currentDecision, 'status') : "",
    ...
    return (
      ...
      >
        ...
        <FormItem {...formItemLayout} label="Scientific name" name="scientificName" >
        <AutoComplete dataSource={_.get(currentRow, `usage.name.scientificName`) ? [_.get(currentRow, `usage.name.scientificName`)] : []} />
          ...
          <FormItem {...formItemLayout} label="Taxonomic status" name="status">
          <Select style={{ width: 200 }} showSearch>
                <Option key="_null" value={""}>
                    -
                  </Option>
    ...
    { rowsForEdit && rowsForEdit.length > 1 && <Steps 
      ...
      current={current} >
        {rowsForEdit.map((r, index) => <Step title={_.get(r, 'usage.name.scientificName')} icon={ index === current && confirmLoading ? <LoadingOutlined /> : null} />)}
    </Steps>}
    </Col>
        </Row>
