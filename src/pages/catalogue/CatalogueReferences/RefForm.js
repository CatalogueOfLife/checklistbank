...
import { Input, InputNumber, Select, Button, Alert, Row, Col, notification, Form } from 'antd';
...
const RefForm = (props) => {
 ...
 const submitData = (values)=> {
    ...
    const task = (id) ? axios.put(`${config.dataApi}dataset/${datasetKey}/reference/${id}`, values /* , conf */) : axios.post(`${config.dataApi}dataset/${datasetKey}/reference`, values/* , conf */);
  ...
  const initialValues= {author: [], editor: [], issue: 1, edition: 1, volume: 1, ...initialData}

  return (

      
    <Form form={form} onFinish={handleSubmit} onFinishFailed={onFinishFailed} initialValues={initialValues} style={{paddingTop: '12px'}}>
    {submissionError && <FormItem><Alert message={<ErrorMsg error={submissionError}></ErrorMsg>} type="error" /></FormItem>}
      ...
      >
        <Select style={{ width: 200 }} onChange={value => setType(value)} showSearch>
