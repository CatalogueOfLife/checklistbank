...
const DatasetIssues = ({ datasetKey, user, issue, issueMap, projectKey }) => {
    ...
    axios(
      ...
      .then((res) => {
        ...
        _.each(_.get(res, "data[0].issuesCount"), (v, k) => {
          tableData.push({ title: k, count: v, datasetKey });
        });
        setLoading(false);
        setData(tableData);
        setError(null);
  ...
  const columns = issueMap ? getColumns({ issueMap, projectKey }) : [];
        ...
        <Table
          ...
          dataSource={
            resolvedSelectedGroups && data
              ? data.filter((d) =>
                  resolvedSelectedGroups.includes(groupMap[d.title])
                )
              : data
          }
          loading={loading}
